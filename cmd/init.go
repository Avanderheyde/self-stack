...
var initCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Check Docker Compose — accept v2 plugin OR legacy v1 standalone.
		composeOK, composeVer := remoteComposeVersion(rc.SSHTarget())
		if !composeOK {
			return fmt.Errorf("Docker Compose not found on VPS\nInstall either:\n  - v2 plugin: apt install docker-compose-plugin\n  - v1 legacy: apt install docker-compose")
		}
		...
		if _, err := ssCheck.CombinedOutput(); err != nil {
			...
			if strings.ToLower(strings.TrimSpace(answer)) != "n" {
				...
				installURL := initInstallURL
				if installURL == "" {
					installURL = os.Getenv("SELFSTACK_INSTALL_URL")
				}
				if installURL == "" {
					installURL = "https://raw.githubusercontent.com/Avanderheyde/self-stack/main/scripts/install.sh"
				}
				installCmd := exec.Command("ssh", rc.SSHTarget(), "curl -fsSL "+shellQuote(installURL)+" | sh")
				installCmd.Stdout = os.Stdout
				installCmd.Stderr = os.Stderr
				if err := installCmd.Run(); err != nil {
					return fmt.Errorf("selfstack installation failed: %w\nInstall manually: curl -fsSL %s | sh", err, installURL)
				}
...
// remoteComposeVersion probes the VPS for docker compose v2 (preferred) and
// falls back to legacy v1. Returns (ok, versionString).
func remoteComposeVersion(sshTarget string) (bool, string) {
	for _, probe := range []string{"docker compose version --short", "docker-compose version --short"} {
		out, err := exec.Command("ssh", sshTarget, probe).CombinedOutput()
		if err == nil {
...
func init() {
	initCmd.Flags().StringVar(&initInstallURL, "install-url", "", "Install script URL to use when bootstrapping selfstack on the VPS")
