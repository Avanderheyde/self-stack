...
func TestOpen_MigratesSourceTypeColumn(t *testing.T) {
	dir := t.TempDir()
	...
	_, err = db.Exec(`
		...
		INSERT INTO apps (name, display_name, repo_url, host_port, status)
		VALUES ('oldapp', 'Old App', 'local:/tmp/oldapp', 10001, 'running');
	`)
	if err != nil {
		t.Fatal(err)
	}
	...
	app, err := s.GetApp("oldapp")
	if err != nil {
		t.Fatal(err)
	}
	if app.SourceType != "registry" {
		t.Fatalf("SourceType = %q, want registry", app.SourceType)
	}
}
