...
func (s *Store) GetApp(name string) (App, error) {
	var a App
	var description, repoURL, version sql.NullString
	err := s.db.QueryRow(
		`SELECT name, display_name, description, repo_url, version, host_port, status, source_type
		 FROM apps WHERE name = ?`, name,
	).Scan(&a.Name, &a.DisplayName, &description, &repoURL, &version, &a.HostPort, &a.Status, &a.SourceType)
...
func (s *Store) ListApps() ([]App, error) {
	...
	for rows.Next() {
		var a App
		var description, repoURL, version sql.NullString
		if err := rows.Scan(&a.Name, &a.DisplayName, &description, &repoURL, &version, &a.HostPort, &a.Status, &a.SourceType); err != nil {
