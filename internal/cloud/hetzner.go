...
type ServerConfig struct {
	ServerType string // e.g., "cax11" (~$4/mo, ARM, 2 vCPU, 4GB RAM)
...
// defaultInstallScriptURL is the URL the cloud-init script curls to install
// selfstack on a fresh VPS. Override with SELFSTACK_INSTALL_URL or the CLI
// --install-url flag while dogfooding unreleased branches.
const defaultInstallScriptURL = "https://raw.githubusercontent.com/Avanderheyde/self-stack/main/scripts/install.sh"

func installScript() string {
	if url := os.Getenv("SELFSTACK_INSTALL_URL"); url != "" {
		return url
	}
...
func SetupScript(tailscaleAuthKey string) string {
	return SetupScriptWithInstallURL(tailscaleAuthKey, "")
}

// SetupScriptWithInstallURL generates cloud-init user-data using a specific
// install script URL. Empty installURL uses SELFSTACK_INSTALL_URL or the default.
func SetupScriptWithInstallURL(tailscaleAuthKey, installURL string) string {
	if installURL == "" {
		installURL = installScript()
	}
	script := `#!/bin/bash
set -euo pipefail
...
curl -fsSL ` + shellQuote(installURL) + ` | sh
